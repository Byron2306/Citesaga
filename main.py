...
class Game:
    ...
    def load_sound(self, name):
        ...
        path = os.path.join('assets', 'sounds', sound_name)
        if not os.path.isfile(path):
    ...
    def load_music(self, name):
        music_name = f"{os.path.splitext(name)[0]}.ogg"
        path = os.path.join('assets', 'music', music_name)
        if not os.path.isfile(path):
            return
    ...
    def activate_terraform(self, player):
        ...
        sound_name = new_card_name.replace('.png', '.ogg')
